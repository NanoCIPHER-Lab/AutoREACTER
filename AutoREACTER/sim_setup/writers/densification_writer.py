...
class DensificationWriter:
    """
    Generates LAMMPS input scripts and supporting files for the densification stage
    of a polymer simulation workflow.

    This class creates an initial low-density empty box, inserts molecules randomly,
    minimizes the system, and then performs a controlled densification using box
    deformation (erate) until the target density is reached.
    """
    ...
    def __init__(
        ...
        """
        Initialize the densification writer with simulation parameters.
        ...
        self.erate = -0.001          # Negative strain rate for box contraction (densification)
        self.timestep = 0.01         # LAMMPS timestep in picoseconds
    ...
    def write_lammps_densification_file(self, replica: Replica) -> str:
        ...
        1. Creates an empty low-density box
        2. Reads force field and molecule templates
        3. Randomly inserts monomers
        4. Minimizes the system
        5. Runs NVT dynamics with box deformation until target density is reached
        ...
        # Add optional neighbor settings if defined
        if s.neighbor:
    ...
    def _get_force_field_types(self) -> dict:
        """
        Parses the LAMMPS force field data file to extract the number of atom,
        bond, angle, dihedral, and improper types. These counts are used with
        the 'extra/*' keywords when reading the initial empty box.

        Returns:
            dict: Number of each force field type

        Raises:
            FileNotFoundError: If the force field data file doesn't exist
            ValueError: If required type headers are missing
        """
        ...
        with open(data_file, 'r') as f:
            for line in f:
                ...
                # Stop parsing once we reach the section with actual data
    ...
    def _run_calculation(self) -> tuple[int, int]:
        """
        Calculates the number of timesteps needed to densify the system from
        25% of target density to the final target density using box deformation.

        The calculation is based on:
            L_final / L_initial = (V_final / V_initial)^(1/3) = (0.25)^(1/3)
            time = (L_ratio - 1) / erate

        A buffer factor is applied and the thermo output interval is adjusted
        based on the total number of steps for performance and monitoring.

        Returns:
            tuple: (total_steps, thermo_interval)
        """
        ...
        if steps_needed <= 0:
            raise ValueError(
                f"Calculated negative steps needed for densification: {steps_needed}. "
                "Check erate and timestep values."
            )

        # Dynamically adjust rounding and thermo frequency based on simulation length
        if steps_needed >= 10000:
            round_steps = round(steps_needed * factor, -4)   # nearest 10,000
            thermo_interval = 1000
        elif steps_needed >= 1000:
            round_steps = round(steps_needed * factor, -3)   # nearest 1,000
            thermo_interval = 100
        elif steps_needed >= 100:
            round_steps = round(steps_needed * factor, -2)   # nearest 100
            thermo_interval = 10
        elif steps_needed >= 10:
            round_steps = round(steps_needed * factor, -1)   # nearest 10
            thermo_interval = 1
        else:
            round_steps = round(steps_needed * factor)
            thermo_interval = 1
    ...
    def _copy_required_files(self, dest_dir: Path) -> None:
        """
        Copies the force field data file and all molecule template files
        into the densification directory so the LAMMPS script can find them.

        Args:
            dest_dir: Destination directory for the copied files
        """
        rf = self.reacter_files
