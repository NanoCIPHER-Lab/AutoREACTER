...
class REACTERFilesBuilder:
    ...
    def _map_file_write(self, reactant_to_product, initiator_atoms, edge_atoms, delete_ids, file_name: str):
        ...
        current_time = datetime.datetime.now().strftime("%Y-%m-%d %H:%M:%S")
        ff_name = getattr(self, 'force_field', 'selected')
        
        # 2. Build the custom header
        map_file = f"# Generated on {current_time}\n"
        map_file += f"# Generated by AutoREACTER for {self.force_field} forcefield\n"
        map_file += f"# File: {file_name}\n"
