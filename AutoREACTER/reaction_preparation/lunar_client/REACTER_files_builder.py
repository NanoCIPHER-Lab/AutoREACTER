...
class REACTERFilesBuilder:
    ...
    def _molecule_file_format(self, number_of_types: int, number_of_bonds: int, number_of_angles: int, 
                            ...
                            bond_section: str, angle_section: str, dihedral_section: str, improper_section: str, file_name: str)-> str:
        """
        ...
        modified_molecule_file = f"""HEADER:{file_name} Generated on {current_time} # Generated by AutoREACTER for Class: 2 > {self.force_field} forcefield AutoREACTER Template Molecule File\n
    ...
    def _molecule_file_preparation(self, template_file_path: Path, template_indexes: list[int], file_name: str) -> tuple[str, dict]:
        """
        ...
        modified_molecule_file = self._molecule_file_format(number_of_types, number_of_bonds, number_of_angles, 
                                                    ...
                                                    bond_section, angle_section, dihedral_section, improper_section, file_name)
        
        return modified_molecule_file, index_change_dict

    def _map_file_write(self, reactant_to_product, initiator_atoms, edge_atoms, delete_ids, file_name: str):
        """
        ...
        """
        #1.  Header giving a nominal description
        current_time = datetime.datetime.now().strftime("%Y-%m-%d %H:%M:%S")
        ...
        map_file += f"{len(reactant_to_product)} equivalences\n"

        # Optionally include a deleteIDs comment line
        if delete_ids:
        ...
        map_file += "\nInitiatorIDs\n\n"
        # 3. Sort initiators for clean output
        for atom in sorted(initiator_atoms):
            map_file += f"{atom+1}\n"
            
        map_file += "\nEdgeIDs\n\n"
        # 4. Sort edge atoms numerically
        for atom in sorted(edge_atoms):
            map_file += f"{atom+1}\n"

        map_file += "\nEquivalences\n\n"
        # 5. Sort the equivalences dictionary by the reactant index (the key)
        for key, value in sorted(reactant_to_product.items()):
            map_file += f"{(key + 1):<5} {value + 1}\n"

        # If delete IDs are present, output them sorted
        if delete_ids:
            map_file += "\nDeleteIDs\n\n"
            for atom in sorted(delete_ids):
                map_file += f"{atom+1}\n"
    ...
    def _build_bond_react_templates(self,
                ...
                for name, pre_path in list(file_dict.items()):
                    ...
                    pre_file_name = f"template_pre_{num}.molecule"
                    post_file_name = f"template_post_{num}.molecule"
                    map_file_name = f"RXN_{num}.map"
    ...
    def molecule_template_preparation(self, lunar_files: LunarFiles, prepared_reactions_with_3d_mols: list[ReactionMetadata]) -> REACTERFiles:
        ...
        for rxn in pre_and_post_files:
            ...
            delete_atoms = getattr(current_rxn_metadata, "delete_atom", False)

            # Extract integer lists from DataFrame columns using helper _col_int_list
            byproducts = self._col_int_list("byproduct_idx", df = df)
